...
let ToolsPalette = class ToolsPalette {
    ...
    redraw() {
        ...
        const buffer = (0, contextSpriteBatch_1.drawCanvas)(width, height, sprites.paletteSpriteSheet, BG, batch => {
            const sprite = sprites[this.spriteName];
            if (sprite) {
                const palette = paletteManager.add(this.palette.map(x => (0, color_1.parseColor)(x.current)));
...
exports.ToolsPalette = ToolsPalette = tslib_1.__decorate([
    (0, core_1.Component)({
