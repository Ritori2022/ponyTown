...
const DEFAULT_STATE = (0, ponyHelpers_1.defaultPonyState)();
const DEFAULT_OPTIONS = (0, ponyHelpers_1.defaultDrawPonyOptions)();
let CharacterPreview = class CharacterPreview {
    ...
    draw() {
        ...
        (0, canvasUtils_1.resizeCanvasWithRatio)(canvas, width, height, false);
        const scale = this.scale * (0, canvasUtils_1.getPixelRatio)();
        const bufferWidth = Math.round(canvas.width / scale);
        const bufferHeight = Math.round(canvas.height / scale);
        if (!bufferWidth || !bufferHeight)
            return;
        this.batch = this.batch || new contextSpriteBatch_1.ContextSpriteBatch((0, canvasUtils_1.createCanvas)(bufferWidth, bufferHeight));
        (0, canvasUtils_1.resizeCanvas)(this.batch.canvas, bufferWidth, bufferHeight);
        ...
        if (this.pony) {
            ...
            try {
                ...
                (0, ponyDraw_1.drawPony)(this.batch, (0, ponyInfo_1.toPalette)(this.pony), this.state || DEFAULT_STATE, x, y, options);
            }
            catch (e) {
                console.error(e);
            }
        ...
        if (this.pony && this.noShadow && this.noBackground && !this.noOutline) {
            ...
            viewContext.fillStyle = (0, color_1.colorToCSS)(colors_1.GRASS_COLOR);
        ...
        if (!this.noShadow && this.name) {
            const name = (0, emoji_1.replaceEmojis)(this.name);
            const scale = 2 * (0, canvasUtils_1.getPixelRatio)();
            const nameBufferWidth = Math.round(canvas.width / scale);
            this.nameBatch = this.nameBatch || new contextSpriteBatch_1.ContextSpriteBatch((0, canvasUtils_1.createCanvas)(nameBufferWidth, 25));
            (0, canvasUtils_1.resizeCanvas)(this.nameBatch.canvas, nameBufferWidth, 25);
            this.nameBatch.start(sprites_1.paletteSpriteSheet, colors_1.TRANSPARENT);
            (0, graphicsUtils_1.drawNamePlate)(this.nameBatch, name, nameBufferWidth / 2, 11, graphicsUtils_1.DrawNameFlags.None, graphicsUtils_1.commonPalettes, this.tag);
            this.nameBatch.end();
            viewContext.save();
            viewContext.scale(scale, scale);
...
exports.CharacterPreview = CharacterPreview = tslib_1.__decorate([
    (0, core_1.Component)({
