...
function createServerActionsFactory(server, settings, getSettings, socketStats) {
    const reportInviteLimitFunc = (0, reporting_1.reportInviteLimit)(admin_accounts_1.reportInviteLimitAccount, `Party invite limit`);
    const reportFriendLimitFunc = (0, reporting_1.reportInviteLimit)(admin_accounts_1.reportFriendLimitAccount, `Friend request limit`);
    ...
    let world;
    const hiding = new hiding_1.HidingService(constants_1.UNHIDE_TIMEOUT, notifications, accountId => (0, world_1.findClientByAccountId)(world, accountId), logger_1.log);
    ...
    const logChatMessage = (client, text, type, ignored, target) => (0, logger_1.chat)(server, client, text, type, ignored, target);
    world.season = constants_1.SEASON;
    world.holiday = constants_1.HOLIDAY;
    try {
        const hidingData = fs.readFileSync((0, hiding_1.hidingDataPath)(server.id), 'utf8');
        if (hidingData) {
            hiding.deserialize(hidingData);
        }
    }
    catch { }
    (0, hiding_1.pollHidingDataSave)(hiding, server.id);
    ...
    const commands = (0, commands_1.createCommands)(world);
    const spamCommands = (0, commands_1.getSpamCommandNames)(commands);
    const runCommand = (0, commands_1.createRunCommand)({ world, notifications, random: lodash_1.random, liveSettings: liveSettings_1.liveSettings, party }, commands);
    ...
    const accountService = {
        ...
        updateCharacterState: (characterId, state) => (0, characterUtils_1.updateCharacterState)(characterId, server.id, state),
    };
    const reportSwears = (0, reporting_1.createReportSwears)(swearsCounter, admin_accounts_1.reportSwearingAccount, admin_accounts_1.timeoutAccount);
    const reportForbidden = (0, reporting_1.createReportForbidden)(forbiddenCounter, admin_accounts_1.timeoutAccount);
    const reportSuspicious = (0, reporting_1.createReportSuspicious)(suspiciousCounter);
    const checkSpam = (0, spamChecker_1.createSpamChecker)(spamCounter, rapidCounter, admin_accounts_1.reportSpammingAccount, admin_accounts_1.timeoutAccount);
    const isSuspiciousMessage = (0, security_1.createIsSuspiciousMessage)(settings);
    const say = (0, chat_1.createSay)(world, runCommand, logChatMessage, checkSpam, reportSwears, reportForbidden, reportSuspicious, spamCommands, Math.random, isSuspiciousMessage);
    const move = (0, move_1.createMove)(teleportCounter);
    const ignorePlayer = (0, playerUtils_1.createIgnorePlayer)(db_1.updateAccount);
    async function createServerActions(client) {
        const { account } = client.tokenData;
        const [friendIds, hideIds] = await Promise.all([(0, db_1.findFriendIds)(account._id), (0, db_1.findHideIds)(account._id)]);
        (0, playerUtils_1.createClientAndPony)(client, friendIds, hideIds, server, world, statesCounter);
