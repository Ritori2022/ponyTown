...
const createReportSuspicious = (counter) => (client, message, suspicious) => {
    const { accountId, account, reporter, shadowed } = client;
    const limit = 5;
    const { count, items } = counter.add(accountId, message);
    if (count > limit || suspicious === 2 /* Suspicious.Very */) {
        const msg = items.join('\n');
        counter.remove(accountId);
        if (!((0, adminUtils_1.isMuted)(account) || shadowed)) {
            reporter.warn('Suspicious messages', msg);
        }
    }
};
exports.createReportSuspicious = createReportSuspicious;
const createReportSwears = (counter, reportSwearing, timeoutAccount, handlePromise = serverUtils_1.handlePromiseDefault) => (client, message, settings) => {
    const { accountId, account, reporter, shadowed } = client;
    const limit = 5; // isNew ? 3 : 6;
    const { count, items } = counter.add(accountId, message);
    if (count > limit) {
        const msg = items.join('\n');
        const timeout = settings.autoBanSwearing && !((0, adminUtils_1.isMuted)(account) || shadowed);
        ...
        handlePromise(Promise.resolve()
            ...
            .then(() => timeout ? timeoutAccount(accountId, (0, utils_1.fromNow)(duration), 'Timed out for swearing') : undefined)
            .then(() => {
            if (timeout) {
                reporter.system('Timed out for swearing', msg, !!settings.reportSwears);
            }
            else if (!((0, adminUtils_1.isMuted)(account) || shadowed)) {
...
const createReportForbidden = (counter, timeoutAccount, handlePromise = serverUtils_1.handlePromiseDefault) => (client, message, settings) => {
    const { accountId, account, reporter, shadowed } = client;
    const newAccount = (0, accountUtils_1.isNew)(account);
    const limit = newAccount ? 5 : 10;
    const { count, items } = counter.add(accountId, message);
    const mutedOrShadowed = (0, adminUtils_1.isMuted)(account) || shadowed;
    if (!mutedOrShadowed) {
        if (count >= limit) {
            const msg = items.join('\n');
            ...
            if (newAccount || settings.autoBanSwearing) {
                handlePromise(timeoutAccount(accountId, (0, utils_1.fromNow)(duration))
...
const reportInviteLimit = (reportInviteLimitAccount, message, handlePromise = serverUtils_1.handlePromiseDefault) => ({ accountId, reporter }) => handlePromise(reportInviteLimitAccount(accountId)
    .then(count => {
