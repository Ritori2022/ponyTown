...
exports.createRemovedDocument = exports.servers = exports.adminServer = exports.loginServers = exports.serverStatus = void 0;
...
const request = tslib_1.__importStar(require("request-promise"));
...
function findServer(id) {
    return (0, utils_1.findById)(exports.servers, id);
}
...
async function join(joinServer, account, character) {
    try {
        const kicked = await mapGameServers(s => {
            if ((0, accountUtils_1.isMod)(account) && s !== joinServer) {
                return false;
            }
            else {
...
function init(world, tokens) {
    accountChangedHandler = (0, internal_1.createAccountChanged)(world, tokens, db_1.findAccountSafe);
}
...
async function accountAround(accountId) {
    ...
    return (0, utils_1.flatten)(users).sort((a, b) => a.distance - b.distance).slice(0, 10);
}
async function accountHidden(accountId) {
    const [users, permaHidden, permaHiddenBy] = await Promise.all([
        ...
        (0, db_1.findHideIds)(accountId),
        (0, db_1.findHideIdsRev)(accountId),
    ]);
    return {
        account: accountId,
        hidden: (0, lodash_1.uniq)((0, lodash_1.flatMap)(users, u => u.hidden)),
        hiddenBy: (0, lodash_1.uniq)((0, lodash_1.flatMap)(users, u => u.hiddenBy)),
        permaHidden,
        permaHiddenBy,
    };
}
const createRemovedDocument = (endPoints, adminService) => (model, id) => {
