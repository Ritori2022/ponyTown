...
function encodeItems(items, base, encode) {
    const baseValues = (0, adminEncoders_1.getBaseTimes)(base);
    return items.map(i => encode(i, baseValues));
}
const events = (0, liveEndPoint_1.createLiveEndPoint)({
    model: db_1.Event,
    fields: adminInterfaces_1.eventFields,
    encode(items, base) {
        base.createdAt = (0, adminEncoders_1.getBaseDate)(items, i => i.createdAt);
        base.updatedAt = (0, adminEncoders_1.getBaseDate)(items, i => i.updatedAt);
...
async function getChat(search, date, caseInsensitive) {
    ...
    async function fetchChatlog(lines) {
        ...
        const { stdout } = await (0, serverUtils_1.execAsync)(`grep ${flags}"${query}" "${logFile}" | tail -n ${lines}`, options);
        return stdout;
    }
    try {
        if (!search) {
            ...
            const { stdout } = await (0, serverUtils_1.execAsync)(`for f in ${paths.pathTo('logs')}/*.log; do `
                + `echo "$f" | grep -o '[0-9]*';`
...
async function getChatForAccounts(accountIds, date) {
    ...
    const map = new Map();
    const ids = (0, utils_1.flatten)(accounts.map(a => [a._id.toString(), ...(a.merges || []).map(a => a.id)]));
...
async function clearSessions(accountId) {
    const clearIds = [];
    await (0, db_1.iterate)(db_1.Session.find({ session: { $exists: true } }).lean(), session => {
        try {
            if (session.session) {
...
async function getAccountDetails(accountId) {
    const [account, invitesReceived, invitesSent] = await Promise.all([
        (0, db_1.findAccount)(accountId, 'merges supporterLog banLog state'),
...
async function updateServerSettings(currentSettings, update) {
    const settings = await Promise.resolve((0, settings_1.loadSettings)());
    Object.assign(currentSettings, settings, update);
    await (0, settings_1.saveSettings)(currentSettings);
