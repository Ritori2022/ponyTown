...
exports.AdminServerActions = void 0;
...
let AdminServerActions = class AdminServerActions {
    constructor(client, server, settings, adminService, endPoints, removedDocument) {
        ...
        this.subscriptions.set('account:deleted', this.adminService.accountDeleted.subscribe(account => {
            if (this.cache.findAccounts) {
                (0, utils_1.removeItem)(this.cache.findAccounts.result, account);
            }
        }));
    }
    disconnected() {
    ...
    async getSignedAccount() {
        return (0, serverUtils_1.toAccountData)(this.account);
    }
    async getCounts() {
    ...
    async getOtherStats() {
        return await (0, admin_1.getOtherStats)(this.adminService);
    }
    ...
    async clearSessions(accountId) {
        await (0, admin_1.clearSessions)(accountId);
    }
    async getState() {
        return (0, admin_1.getAdminState)();
    }
    async updateSettings(update) {
        await (0, admin_1.updateServerSettings)(this.settings, update);
    }
    async updateGameServerSettings(serverId, update) {
        await (0, admin_1.updateGameServerSettings)(this.settings, serverId, update);
    }
    async fetchServerStats(serverId) {
        const server = (0, internal_1.getServer)(serverId);
    ...
    async report(accountId) {
        (0, reporter_1.create)(this.server, accountId).info(`Reported by ${this.account.name}`);
    }
    async notifyUpdate(server) {
        await (0, admin_1.notifyUpdate)(server);
    }
    async shutdownServers(server) {
        await (0, admin_1.shutdownServers)(server, true);
    }
    async resetUpdating(server) {
        await (0, admin_1.resetUpdating)(server);
    }
    async action(action, accountId) {
        await (0, admin_1.actionForAllServers)(action, accountId);
    }
    async kick(accountId) {
        await (0, admin_1.kickFromAllServers)(accountId);
    }
    ...
    async getChat(search, date, caseInsensitive) {
        return await (0, admin_1.getChat)(search, date, caseInsensitive);
    }
    async getChatForAccounts(accountIds, date) {
        return await (0, admin_1.getChatForAccounts)(accountIds, date);
    }
    async getRequestStats() {
        const loginServer = (0, internal_1.getLoginServer)('login');
        const requests = await loginServer.api.loginServerStats();
        const userCounts = await (0, admin_1.getUserCounts)();
    ...
    async updateOrigin(origin) {
        await (0, admin_1.updateOrigin)(origin);
    }
    async getOriginStats() {
        return await (0, origins_1.getOriginStats)(this.adminService.accounts.items);
    }
    async clearOrigins(count, andHigher, options) {
        if (!this.adminService.loaded) {
            ...
            await (0, origins_1.clearOrigins)(this.adminService, count, andHigher, options);
        }
    }
    async clearOriginsForAccounts(accounts, options) {
        if (!this.adminService.loaded) {
            ...
            await (0, origins_1.clearOriginsForAccounts)(this.adminService, accounts, options);
        }
    }
    // ponies
    async getPony(id) {
    ...
    async getPonyInfo(id) {
        const character = await (0, db_1.findCharacterById)(id);
        return (0, serverUtils_1.toPonyObjectAdmin)(character);
    }
    ...
    async getDetailsForAccount(accountId) {
        return await (0, admin_1.getAccountDetails)(accountId);
    }
    async findPonies(query, page, _skipTotalCount) {
        return await (0, ponies_1.findPonies)(query, page);
    }
    async createPony(account, name, info) {
        await (0, ponies_1.createCharacter)(account, name, info);
        (0, logger_1.system)(account, `Created character (${name}) ${this.by()}`);
    }
    async assignPony(ponyId, accountId) {
        await (0, ponies_1.assignCharacter)(ponyId, accountId);
    }
    async removePony(id) {
        await (0, ponies_1.removeCharacter)(this.adminService, id);
    }
    async removePoniesAboveLimit(account) {
        await (0, ponies_1.removeCharactersAboveLimit)(this.removedDocument, account);
        (0, logger_1.system)(account, `Removed ponies above limit ${this.by()}`);
    }
    async removeAllPonies(account) {
        await (0, ponies_1.removeAllCharacters)(this.removedDocument, account);
        (0, logger_1.system)(account, `Removed all ponies ${this.by()}`);
    }
    // auths
    async getAuth(id) {
    ...
    async fetchAuthDetails(auths) {
        const items = await (0, db_1.queryAuths)({ _id: { $in: auths } }, '_id lastUsed');
        return items.map(a => ({
            id: a._id.toString(),
    ...
    async updateAuth(id, update) {
        ...
        await (0, db_1.updateAuth)(id, update);
    }
    async assignAuth(authId, accountId) {
        await (0, admin_auths_1.assignAuth)(authId, accountId);
    }
    async removeAuth(id) {
        await (0, admin_auths_1.removeAuth)(this.adminService, id);
    }
    // accounts
    async getAccount(id) {
        return await (0, db_1.findAccount)(id);
    }
    async findAccounts(query) {
        return await (0, admin_accounts_1.findAccounts)(this.cache, this.adminService, query);
    }
    async createAccount(name) {
        const account = await db_1.Account.create({ name });
        (0, logger_1.system)(account._id.toString(), `Created account ${this.by()}`);
        return account._id.toString();
    }
    async getAccountsByEmails(emails) {
        return (0, admin_accounts_1.getAccountsByEmails)(this.adminService, emails);
    }
    async getAccountsByOrigin(ip) {
        return (0, admin_accounts_1.getAccountsByOrigin)(this.adminService, ip);
    }
    async setName(accountId, name) {
        await (0, admin_accounts_1.updateAccountSafe)(accountId, { name });
        (0, logger_1.system)(accountId, `Updated name (${name}) ${this.by()}`);
    }
    async setAge(accountId, age) {
        ...
        (0, logger_1.system)(accountId, `Updated birth year (${age}) ${this.by()}`);
    }
    async setRole(accountId, role, set) {
        await (0, admin_accounts_1.setRole)(accountId, role, set, (0, accountUtils_1.hasRole)(this.account, 'superadmin'));
        (0, logger_1.system)(accountId, `${set ? 'Added' : 'Removed'} role (${role}) ${this.by()}`);
    }
    async updateAccount(accountId, update, message) {
        await (0, admin_accounts_1.updateAccountSafe)(accountId, update);
        if (message) {
            (0, logger_1.system)(accountId, `${message} ${this.by()}`);
        }
    }
    async timeoutAccount(accountId, timeout) {
        const message = timeout ? `Timed out ${moment.duration(timeout).humanize()}` : 'Unmuted';
        (0, logger_1.system)(accountId, `${message} ${this.by()}`);
        await (0, admin_accounts_1.timeoutAccount)(accountId, (0, utils_1.fromNow)(timeout | 0));
    }
    async updateAccountCounter(accountId, name, value) {
        await (0, admin_accounts_1.updateAccountCounter)(accountId, name, value);
    }
    async mergeAccounts(accountId, withId) {
        const server = (0, internal_1.getLoginServer)('login');
        await server.api.mergeAccounts(accountId, withId, this.by(), (0, accountUtils_1.hasRole)(this.account, 'superadmin'), true);
    }
    async unmergeAccounts(accountId, mergeId, split, keep) {
        await (0, merge_1.splitAccounts)(accountId, mergeId, split, keep, this.by());
    }
    async getAccountStatus(accountId) {
        return await (0, internal_1.accountStatus)(accountId);
    }
    async getAccountAround(accountId) {
        return await (0, internal_1.accountAround)(accountId);
    }
    async getAccountHidden(accountId) {
        return await (0, internal_1.accountHidden)(accountId);
    }
    async getAccountFriends(accountId) {
        return (0, db_1.findFriendIds)(accountId);
    }
    async removeAccount(accountId) {
        await (0, admin_accounts_1.removeAccount)(this.adminService, accountId);
    }
    async setAlert(accountId, message, expiresIn) {
        await (0, admin_accounts_1.setAccountAlert)(accountId, message, (0, utils_1.fromNow)(expiresIn));
        (0, logger_1.system)(accountId, `${expiresIn ? 'Set' : 'Unset'} alert for ${(0, utils_1.formatDuration)(expiresIn)} "${message}" ${this.by()}`);
    }
    // accounts - origins
    async removeAllOrigins(accountId) {
        await (0, origins_1.removeAllOrigins)(this.adminService, accountId);
    }
    async removeOriginsForAccount(accountId, ips) {
        await (0, origins_1.removeOrigins)(this.adminService, accountId, ips);
    }
    async removeOriginsForAccounts(origins) {
        await Promise.all(origins.map(o => (0, origins_1.removeOrigins)(this.adminService, o.accountId, o.ips)));
    }
    async addOriginToAccount(accountId, origin) {
        if (origin && origin.ip && origin.country) {
            await (0, origins_1.addOrigin)(accountId, origin);
            (0, logger_1.system)(accountId, `Added origin (${JSON.stringify(origin)}) ${this.by()}`);
        }
        else {
    ...
    async addEmail(accountId, email) {
        await (0, admin_accounts_1.addEmail)(accountId, email);
        (0, logger_1.system)(accountId, `Added email (${email}) ${this.by()}`);
    }
    async removeEmail(accountId, email) {
        await (0, admin_accounts_1.removeEmail)(accountId, email);
        (0, logger_1.system)(accountId, `Removed email (${email}) ${this.by()}`);
    }
    // accounts - ignores
    async removeIgnore(accountId, ignore) {
        await (0, admin_accounts_1.removeIgnore)(accountId, ignore);
    }
    async addIgnores(accountId, ignores) {
        await (0, admin_accounts_1.addIgnores)(accountId, ignores);
    }
    async setAccountState(accountId, state) {
        await (0, admin_accounts_1.setAccountState)(accountId, state);
    }
    ...
    async removeFriend(accountId, friendId) {
        await (0, accountUtils_2.removeFriend)(accountId, friendId);
    }
    async addFriend(accountId, friendId) {
        await (0, accountUtils_2.addFriend)(accountId, friendId);
    }
    // accounts - duplicates
    async getAllDuplicatesQuickInfo(accountId) {
        return await (0, duplicates_1.getAllDuplicatesQuickInfo)(this.adminService, accountId);
    }
    async getAllDuplicates(accountId) {
        return await (0, duplicates_1.getAllDuplicatesWithInfo)(this.adminService, accountId);
    }
    async getDuplicateEntries(force) {
        return await (0, duplicates_1.getDuplicateEntries)(this.adminService.accounts.items, force);
    }
    // patreon
    async updatePatreon() {
        await (0, polling_1.updatePatreonData)(this.server, this.settings);
    }
    ...
    async getLastPatreonData() {
        const data = await (0, patreon_1.getLastPatreonData)();
        // if (data) {
    ...
    async updatePastSupporters() {
        await (0, polling_1.updatePastSupporters)();
    }
    // other
    ...
    async teleportTo(accountId) {
        ...
        await (0, admin_1.forAllGameServers)(server => server.api.teleportTo(adminAccountId, accountId));
    }
    // utils
    by() {
...
tslib_1.__decorate([
    (0, ag_sockets_1.Method)({ promise: true }),
...
exports.AdminServerActions = AdminServerActions = tslib_1.__decorate([
...
async function throwOnAdmin(account) {
    if (account) {
        const isAdmin = await (0, db_1.checkIfAdmin)(account);
        if (isAdmin) {
