...
describe('NotificationService', () => {
    ...
    describe('addNotification()', () => {
        it('adds notification to client', () => {
            ...
            (0, chai_1.expect)(client.notifications).contain(notification);
        });
        it('returns new notification ID', () => {
            const notification = { id: 0, name: 'name', message: 'test' };
            (0, chai_1.expect)(notificationService.addNotification(client, notification)).equal(1);
        });
        it('assigns ID to notification', () => {
            ...
            notificationService.addNotification(client, notification);
            (0, chai_1.expect)(notification.id).not.equal(0);
        });
        it('sends addNotification', () => {
            const notification = { id: 0, name: 'name', message: 'test', note: 'note', flags: 123 };
            const addNotification = (0, sinon_1.stub)(client, 'addNotification');
            notificationService.addNotification(client, notification);
        ...
        it('does not add notification to client if limit is reached', () => {
            (0, utils_1.times)(10, () => notificationService.addNotification(client, { id: 0, name: 'name', message: (0, lodash_1.uniqueId)('test') }));
            const addNotification = (0, sinon_1.stub)(client, 'addNotification');
            (0, chai_1.expect)(notificationService.addNotification(client, { id: 0, name: 'name', message: (0, lodash_1.uniqueId)('test') })).equal(0);
            sinon_1.assert.notCalled(addNotification);
        });
        it('does not add notification to client if identical notification already exists', () => {
            const addNotification = (0, sinon_1.stub)(client, 'addNotification');
            (0, chai_1.expect)(notificationService.addNotification(client, { id: 0, name: 'name', message: 'foo' })).not.equal(0);
            (0, chai_1.expect)(notificationService.addNotification(client, { id: 0, name: 'name', message: 'foo' })).equal(0);
            (0, chai_1.expect)(client.notifications.length).equal(1);
    ...
    describe('removeNotification()', () => {
        it('removes notification from client', () => {
            ...
            (0, chai_1.expect)(client.notifications).not.contain(notification);
        });
        it('does nothing if notification does not exist', () => {
            const removeNotification = (0, sinon_1.stub)(client, 'removeNotification');
            notificationService.removeNotification(client, 1);
        ...
        it('returns true if notification is removed', () => {
            client.notifications.push({ id: 1, name: 'name', message: 'test' });
            (0, chai_1.expect)(notificationService.removeNotification(client, 1)).true;
        });
        it('returns false if notification does not exist', () => {
            (0, chai_1.expect)(notificationService.removeNotification(client, 1)).false;
        });
    });
    describe('acceptNotification()', () => {
        it('calls accept callback', () => {
            const accept = (0, sinon_1.spy)();
        ...
        it('works if notification does not have accept callback', () => {
            ...
            notificationService.addNotification(client, notification);
            notificationService.acceptNotification(client, 1);
            (0, chai_1.expect)(client.notifications).not.include(notification);
        });
    });
    describe('rejectNotification()', () => {
        it('calls reject callback', () => {
            const reject = (0, sinon_1.spy)();
    ...
    describe('rejectAll()', () => {
        it('rejects all notifications', () => {
            ...
            const rejectNotification = (0, sinon_1.stub)(notificationService, 'rejectNotification');
