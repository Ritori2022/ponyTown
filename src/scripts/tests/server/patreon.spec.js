...
const dbId = () => mongoose_1.Types.ObjectId((0, mocks_1.genId)());
...
describe('patreon', () => {
    describe('fetchPatreonData()', () => {
        ...
        it('returns rewards', async () => {
            ...
            const result = await (0, patreon_1.fetchPatreonData)(client, lodash_1.noop);
            (0, chai_1.expect)(result.rewards).eql([
        ...
        it('returns pledges', async () => {
            ...
            const result = await (0, patreon_1.fetchPatreonData)(client, lodash_1.noop);
            (0, chai_1.expect)(result.pledges).eql([
        ...
        it('throws on too many pages', async () => {
            ...
            await (0, chai_1.expect)((0, patreon_1.fetchPatreonData)(client, lodash_1.noop)).rejectedWith('Exceeded 100 pages of patreon data');
        });
    });
    describe('updatePatreonInfo()', () => {
        ...
        beforeEach(() => {
            queryAuths = (0, sinon_1.stub)();
            queryAccounts = (0, sinon_1.stub)();
            removeOldSupporters = (0, sinon_1.stub)();
            updateSupporters = (0, sinon_1.stub)();
            addTotalPledged = (0, sinon_1.stub)();
            updatePatreonInfo = (0, patreon_1.createUpdatePatreonInfo)(queryAuths, queryAccounts, removeOldSupporters, updateSupporters, addTotalPledged);
        });
        it('queries auths', async () => {
            ...
            sinon_1.assert.calledWith(queryAuths, (0, sinon_1.match)({
                provider: 'patreon',
                openId: { $in: ['foo', 'bar'] },
                account: { $exists: true },
                banned: { $ne: true },
        ...
        it('queries accounts', async () => {
            ...
            sinon_1.assert.calledWith(queryAccounts, (0, sinon_1.match)({ patreon: { $exists: true, $ne: 0 } }), '_id patreon supporterDeclinedSince');
        });
    ...
    describe('removeOldSupporters()', () => {
        ...
        beforeEach(() => {
            ...
            updateAccounts = (0, sinon_1.stub)();
            log = (0, sinon_1.stub)();
            removeOldSupporters = (0, patreon_1.createRemoveOldSupporters)(updateAccounts, log);
        });
        afterEach(() => {
            clock.restore();
        });
        ...
        it('unsets patreon for all accounts without corresponding auths', async () => {
            ...
            await removeOldSupporters([], [(0, mocks_1.account)({ _id: accountId })]);
        ...
        it('unsets patreon for all accounts without corresponding auths (2)', async () => {
            ...
            await removeOldSupporters([(0, mocks_1.auth)({ account: mongoose_1.Types.ObjectId(account2Id.toHexString()) })], [(0, mocks_1.account)({ _id: account1Id }), (0, mocks_1.account)({ _id: account2Id })]);
        ...
        it('works with unassigned auths', async () => {
            const accountId = dbId();
            await removeOldSupporters([(0, mocks_1.auth)({ account: undefined })], [(0, mocks_1.account)({ _id: accountId })]);
            sinon_1.assert.calledWithMatch(updateAccounts, { _id: { $in: [accountId] } }, update);
        });
    });
    describe('updateSupporters()', () => {
        ...
        beforeEach(() => {
            clock = (0, sinon_1.useFakeTimers)();
            clock.setSystemTime(1234);
            updateAccount = (0, sinon_1.stub)();
            log = (0, sinon_1.stub)();
            updateSupporters = (0, patreon_1.createUpdateSupporters)(updateAccount, log);
        });
        afterEach(() => {
            clock.restore();
        });
        ...
        it('adds patreon info to account', async () => {
            const accountId = dbId();
            await updateSupporters([(0, mocks_1.auth)({ account: accountId, openId: '123' })], [], [{ reward: constants_1.rewardLevel1, user: '123', total: 0 }], new Date());
        ...
        it('handles duplicate auths', async () => {
            const accountId = dbId();
            await updateSupporters([(0, mocks_1.auth)({ account: accountId, openId: '123' }), (0, mocks_1.auth)({ account: accountId, openId: '321' })], [], [{ reward: constants_1.rewardLevel2, user: '321', total: 0 }, { reward: constants_1.rewardLevel1, user: '123', total: 0 }], new Date());
            sinon_1.assert.calledWith(updateAccount, accountId.toString(), {
                patreon: 2 /* PatreonFlags.Supporter2 */,
                supporterDeclinedSince: undefined,
        ...
        it('handles unassigned auths', async () => {
            await updateSupporters([(0, mocks_1.auth)({ account: undefined, openId: '123' })], [], [{ reward: constants_1.rewardLevel2, user: '123', total: 0 }], new Date());
            sinon_1.assert.notCalled(updateAccount);
        });
        it('sets supporter to none if pledge is missing', async () => {
            const accountId = dbId();
            await updateSupporters([(0, mocks_1.auth)({ account: accountId, openId: '123' })], [(0, mocks_1.account)({ _id: accountId, patreon: 1 /* PatreonFlags.Supporter1 */ })], [], new Date());
            sinon_1.assert.calledWith(updateAccount, accountId.toString(), {
                patreon: 0 /* PatreonFlags.None */,
                supporterDeclinedSince: undefined,
        ...
        it('updates existing info', async () => {
            const accountId = dbId();
            await updateSupporters([(0, mocks_1.auth)({ account: accountId, openId: '123' })], [(0, mocks_1.account)({ _id: mongoose_1.Types.ObjectId(accountId.toHexString()), patreon: 1 /* PatreonFlags.Supporter1 */ })], [{ reward: constants_1.rewardLevel2, user: '123', total: 0 }], new Date());
            sinon_1.assert.calledWith(updateAccount, accountId.toString(), {
                patreon: 2 /* PatreonFlags.Supporter2 */,
                supporterDeclinedSince: undefined,
                $push: push('added supporter (2)'),
        ...
        it('does not update if supporter level did not change (2 patreon accounts)', async () => {
            const accountId = dbId();
            await updateSupporters([(0, mocks_1.auth)({ account: accountId, openId: '123' }), (0, mocks_1.auth)({ account: accountId, openId: '321' })], [(0, mocks_1.account)({ _id: mongoose_1.Types.ObjectId(accountId.toHexString()), patreon: 2 /* PatreonFlags.Supporter2 */ })], [{ reward: constants_1.rewardLevel2, user: '123', total: 0 }], new Date());
            sinon_1.assert.notCalled(updateAccount);
        ...
        it('does not remove support if decline is set but day of month is < 7', async () => {
            ...
            const date = (0, utils_1.fromDate)(now, -1 * constants_1.DAY);
            await updateSupporters([(0, mocks_1.auth)({ account: accountId, openId: '123' })], [(0, mocks_1.account)({ _id: mongoose_1.Types.ObjectId(accountId.toHexString()), patreon: 1 /* PatreonFlags.Supporter1 */ })], [{ reward: constants_1.rewardLevel1, user: '123', total: 0, declinedSince: date.toISOString() }], now);
            sinon_1.assert.calledWith(updateAccount, accountId.toString(), {
        ...
        it('removes support if decline is set and day of month is < 14 and decline is > 14 days old', async () => {
            const accountId = dbId();
            const now = new Date('2018-04-02T09:00:00.000Z');
            const date = (0, utils_1.fromDate)(now, -16 * constants_1.DAY);
            await updateSupporters([(0, mocks_1.auth)({ account: accountId, openId: '123' })], [(0, mocks_1.account)({ _id: mongoose_1.Types.ObjectId(accountId.toHexString()), patreon: 1 /* PatreonFlags.Supporter1 */ })], [{ reward: constants_1.rewardLevel1, user: '123', total: 0, declinedSince: date.toISOString() }], now);
            sinon_1.assert.calledWith(updateAccount, accountId.toString(), {
                patreon: 0 /* PatreonFlags.None */,
                supporterDeclinedSince: date,
        ...
        it('does not add log if declined but supporter is already removed', async () => {
            ...
            const now = new Date('2018-04-16T09:00:00.000Z');
            const date = (0, utils_1.fromDate)(now, -1 * constants_1.DAY);
            await updateSupporters([(0, mocks_1.auth)({ account: accountId, openId: '123' })], [], [{ reward: constants_1.rewardLevel1, user: '123', total: 0, declinedSince: date.toISOString() }], now);
            sinon_1.assert.calledWith(updateAccount, accountId.toString(), {
                patreon: 0 /* PatreonFlags.None */,
                supporterDeclinedSince: date,
            });
        ...
        it('updates declined date', async () => {
            const accountId = dbId();
            const date = (0, utils_1.fromNow)(-100);
            await updateSupporters([(0, mocks_1.auth)({ account: accountId, openId: '123' })], [(0, mocks_1.account)({ _id: mongoose_1.Types.ObjectId(accountId.toHexString()), patreon: 1 /* PatreonFlags.Supporter1 */ })], [{ reward: constants_1.rewardLevel2, user: '123', total: 0, declinedSince: date.toISOString() }], new Date());
            sinon_1.assert.calledWith(updateAccount, accountId.toString(), {
                patreon: 2 /* PatreonFlags.Supporter2 */,
                supporterDeclinedSince: date,
                $push: push('added supporter (2)'),
        ...
        it('updates declined date even if patreon is not changed', async () => {
            const accountId = dbId();
            const date = (0, utils_1.fromNow)(-100);
            await updateSupporters([(0, mocks_1.auth)({ account: accountId, openId: '123' })], [(0, mocks_1.account)({ _id: mongoose_1.Types.ObjectId(accountId.toHexString()), patreon: 2 /* PatreonFlags.Supporter2 */ })], [{ reward: constants_1.rewardLevel2, user: '123', total: 0, declinedSince: date.toISOString() }], new Date());
            sinon_1.assert.calledWith(updateAccount, accountId.toString(), {
                supporterDeclinedSince: date,
            });
            sinon_1.assert.notCalled(log);
        ...
        it('sets supporter to none if reward has invalid ID', async () => {
            const accountId = dbId();
            await updateSupporters([(0, mocks_1.auth)({ account: accountId, openId: '123' })], [(0, mocks_1.account)({ _id: mongoose_1.Types.ObjectId(accountId.toHexString()), patreon: 1 /* PatreonFlags.Supporter1 */ })], [{ reward: 'invalid', user: '123', total: 0 }], new Date());
            sinon_1.assert.calledWith(updateAccount, accountId.toString(), {
                patreon: 0 /* PatreonFlags.None */,
                supporterDeclinedSince: undefined,
                $push: push('removed supporter'),
        ...
        it('does nothing if patreon info is already set', async () => {
            const accountId = dbId();
            await updateSupporters([(0, mocks_1.auth)({ account: accountId, openId: '123' })], [(0, mocks_1.account)({ _id: mongoose_1.Types.ObjectId(accountId.toHexString()), patreon: 1 /* PatreonFlags.Supporter1 */ })], [{ reward: constants_1.rewardLevel1, user: '123', total: 0 }], new Date());
            sinon_1.assert.notCalled(updateAccount);
        });
        it('does nothing if declined date is the same', async () => {
            const accountId = dbId();
            await updateSupporters([(0, mocks_1.auth)({ account: accountId, openId: '123' })], [(0, mocks_1.account)({
    ...
    describe('addTotalPledged()', () => {
        ...
        let addTotalPledged;
        beforeEach(() => {
            updateAuth = (0, sinon_1.stub)();
            addTotalPledged = (0, patreon_1.createAddTotalPledged)(updateAuth);
        });
        ...
        it('does nothing if cannot find pledges for auth', async () => {
            await addTotalPledged([(0, mocks_1.auth)({ openId: '123' })], []);
            sinon_1.assert.notCalled(updateAuth);
        });
        it('does nothing if total is already correct', async () => {
            await addTotalPledged([(0, mocks_1.auth)({ openId: '123', pledged: 10 })], [{ reward: 'some', user: '123', total: 10 }]);
            sinon_1.assert.notCalled(updateAuth);
        });
        it('updates total if different', async () => {
            const authId = dbId();
            await addTotalPledged([(0, mocks_1.auth)({ _id: authId, openId: '123' })], [{ reward: 'some', user: '123', total: 10 }]);
