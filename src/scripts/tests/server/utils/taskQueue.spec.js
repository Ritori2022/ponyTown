...
describe('taskQueue', () => {
    ...
    it('executes given action', async () => {
        const action = (0, sinon_1.stub)();
        queue.push(action);
        await queue.wait();
        sinon_1.assert.calledOnce(action);
    });
    it('executes two actions', async () => {
        const action1 = (0, sinon_1.stub)();
        const action2 = (0, sinon_1.stub)();
        queue.push(action1);
        queue.push(action2);
        await queue.wait();
    ...
    it('executes actions in order', async () => {
        const order = [0];
        const action1 = (0, sinon_1.stub)().callsFake(() => order.push(1));
        const action2 = (0, sinon_1.stub)().callsFake(() => order.push(2));
        queue.push(action1);
        queue.push(action2);
        await queue.wait();
        (0, chai_1.expect)(order).eql([0, 1, 2]);
    });
    ...
    it('returns promise that resolves to value returned from action', async () => {
        const action = (0, sinon_1.stub)().returns('foo');
        await (0, chai_1.expect)(queue.push(action)).eventually.equal('foo');
    });
    it('returns promise that resolves to value resolved from action', async () => {
        const action = (0, sinon_1.stub)().resolves('foo');
        await (0, chai_1.expect)(queue.push(action)).eventually.equal('foo');
    });
    it('returns promise that rejects to error thrown from action', async () => {
        const action = (0, sinon_1.stub)().throws(new Error('test'));
        await (0, chai_1.expect)(queue.push(action)).rejectedWith('test');
    });
    it('returns promise that rejects to error rejected from action', async () => {
        const action = (0, sinon_1.stub)().rejects(new Error('test'));
        await (0, chai_1.expect)(queue.push(action)).rejectedWith('test');
    });
    it('returns promise that rejects to error rejected from action', async () => {
        const action = (0, sinon_1.stub)().rejects(new Error('test'));
    ...
    describe('makeQueued()', () => {
        it('creates queued function', async () => {
            const action = (0, sinon_1.stub)();
            const queued = (0, taskQueue_1.makeQueued)(action);
            await queued();
            sinon_1.assert.calledOnce(action);
        });
    });
