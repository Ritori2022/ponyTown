...
describe('api game', () => {
    describe('joinGame()', () => {
        ...
        beforeEach(() => {
            ...
            join = (0, sinon_1.stub)();
            addOrigin = (0, sinon_1.stub)();
            hasInvites = (0, sinon_1.stub)();
            server = { state: { settings: {} } };
            const findServer = (0, sinon_1.stub)();
            findServer.withArgs('serverid').returns(server);
            joinGame = (0, game_1.createJoinGame)(findServer, { version: '1', host: 'http://foo.bar/', debug: false, local: false }, findCharacter, join, addOrigin, hasInvites);
        });
        it('returns join token', async () => {
            const a = (0, mocks_1.account)({ _id: (0, mocks_1.genObjectId)() });
            const character = {};
            findCharacter.withArgs('charid').returns(character);
            join.withArgs(server, a, character).returns('tokenid');
            await (0, chai_1.expect)(joinGame(a, 'charid', 'serverid', '1', 'http://foo.bar/', false, {}))
                .eventually.eql({ token: 'tokenid' });
        });
        it('resolves if meets server requirement', async () => {
            server.state.require = 'sup2';
            const a = (0, mocks_1.account)({ _id: (0, mocks_1.genObjectId)(), patreon: 2 /* PatreonFlags.Supporter2 */ });
            const character = {};
            findCharacter.withArgs('charid').returns(character);
            join.withArgs(server, a, character).returns('tokenid');
        ...
        it('returns alert if has account alert', async () => {
            const a = (0, mocks_1.account)({ _id: (0, mocks_1.genObjectId)(), alert: { message: 'test alert', expires: (0, utils_1.fromNow)(9999) } });
            const origin = {};
            findCharacter.withArgs('charid').returns({});
            const result = await joinGame(a, 'charid', 'serverid', '1', 'http://foo.bar/', false, origin);
            (0, chai_1.expect)(result).eql({ alert: 'test alert' });
        });
        it('does not returl alert if alredy has alert', async () => {
            ...
            (0, chai_1.expect)(result).eql({ token: 'tokenid' });
        });
        it('rejects if passed version is different than server version', async () => {
            await (0, chai_1.expect)(joinGame((0, mocks_1.account)({ _id: (0, mocks_1.genObjectId)() }), 'charid', 'serverid', '2', 'http://foo.bar/', false, {}))
                .rejectedWith(errors_1.VERSION_ERROR);
        });
        it('rejects if passed url is different than server url', async () => {
            await (0, chai_1.expect)(joinGame((0, mocks_1.account)({ _id: (0, mocks_1.genObjectId)() }), 'charid', 'serverid', '1', 'http://im.invalid/', false, {}))
                .rejectedWith('Invalid data');
        });
        it('rejects if server is not found', async () => {
            await (0, chai_1.expect)(joinGame((0, mocks_1.account)({ _id: (0, mocks_1.genObjectId)() }), 'charid', 'doesnotexist', '1', 'http://foo.bar/', false, {}))
                .rejectedWith('Invalid data');
        });
        it('rejects if server is offline', async () => {
            server.state.settings.isServerOffline = true;
            await (0, chai_1.expect)(joinGame((0, mocks_1.account)({ _id: (0, mocks_1.genObjectId)() }), 'charid', 'serverid', '1', 'http://foo.bar/', false, {}))
        ...
        it('rejects if character ID is missing', async () => {
            await (0, chai_1.expect)(joinGame((0, mocks_1.account)({ _id: (0, mocks_1.genObjectId)() }), undefined, 'serverid', '1', 'http://foo.bar/', false, {}))
                .rejectedWith('Invalid data');
        });
        it('rejects if character ID is not string', async () => {
            await (0, chai_1.expect)(joinGame((0, mocks_1.account)({ _id: (0, mocks_1.genObjectId)() }), { foo: 'bar' }, 'serverid', '1', 'http://foo.bar/', false, {}))
                .rejectedWith('Invalid data');
        });
        ...
        it('rejects if already joining', async () => {
            ...
            joinGame((0, mocks_1.account)({ _id }), 'charid', 'serverid', '1', 'http://foo.bar/', false, {});
            await (0, utils_1.delay)(1);
            await (0, chai_1.expect)(joinGame((0, mocks_1.account)({ _id }), 'charid', 'serverid', '1', 'http://foo.bar/', false, {}))
        ...
        it('does not reject if other client is already joining', async () => {
            ...
            joinGame((0, mocks_1.account)({ _id: (0, mocks_1.genObjectId)() }), 'charid2', 'serverid', '1', 'http://foo.bar/', false, {});
            await (0, utils_1.delay)(1);
            await (0, chai_1.expect)(joinGame((0, mocks_1.account)({ _id: (0, mocks_1.genObjectId)() }), 'charid', 'serverid', '1', 'http://foo.bar/', false, {}))
                .eventually.eql({ token: 'tokenid' });
        });
