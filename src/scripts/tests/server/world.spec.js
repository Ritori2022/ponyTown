...
describe('World', () => {
    const partyService = (0, lib_1.stubClass)(party_1.PartyService);
    const friendsService = (0, lib_1.stubClass)(friends_1.FriendsService);
    const hidingService = (0, lib_1.stubClass)(hiding_1.HidingService);
    partyService.partyChanged = new rxjs_1.Subject();
    const notifications = (0, lib_1.stubClass)(notification_1.NotificationService);
    let liveSettings;
    let getSettings;
    let world;
    let map;
    let client;
    beforeEach(() => {
        (0, lib_1.resetStubMethods)(partyService, 'leave');
        (0, lib_1.resetStubMethods)(notifications, 'rejectAll');
        liveSettings = {};
        getSettings = (0, sinon_1.stub)().returns({});
        ...
        world.maps.push(map = (0, serverMap_1.createServerMap)('', 0, 1, 1));
    ...
    describe('joinClientToWorld()', () => {
        ...
        it('sends world state', () => {
            const worldState = (0, sinon_1.stub)(client, 'worldState');
            const state = {};
            (0, sinon_1.stub)(world, 'getState').returns(state);
            world.joinClientToWorld(client);
            sinon_1.assert.calledWith(worldState, state, true);
        });
        it('sends map info', () => {
            const mapState = (0, sinon_1.stub)(client, 'mapState');
            const state = {};
            map.state = state;
            world.joinClientToWorld(client);
        ...
        it('adds client to clients list', () => {
            world.joinClientToWorld(client);
            (0, chai_1.expect)(world.clients).contain(client);
        });
        it('initializes client pony ID', () => {
            (0, sinon_1.stub)(world, 'getNewEntityId').returns(1234);
            world.joinClientToWorld(client);
            (0, chai_1.expect)(client.pony.id).equal(1234);
        });
        it('sends client pony ID', () => {
            (0, sinon_1.stub)(world, 'getNewEntityId').returns(1234);
            const myEntity = (0, sinon_1.stub)(client, 'myEntity');
        ...
        it('adds client pony to the world', () => {
            const addEntity = (0, sinon_1.stub)(world, 'addEntity');
            world.joinClientToWorld(client);
            sinon_1.assert.calledWith(addEntity, client.pony, client.map);
        });
        it('updates selection for other clients', () => {
            const otherClient = (0, mocks_1.mockClient)();
            otherClient.selected = { id: 123, client: { accountId: 'foobar' } };
            const otherClient2 = (0, mocks_1.mockClient)();
            const updateSelection = (0, sinon_1.stub)(otherClient, 'updateSelection');
            const updateSelection2 = (0, sinon_1.stub)(otherClient2, 'updateSelection');
            (0, sinon_1.stub)(world, 'getNewEntityId').returns(321);
        ...
        it('adds update notification to client if updating', () => {
            ...
            sinon_1.assert.calledWith(notifications.addNotification, client, {
                id: 0, name: '', message: 'Server will restart shortly for updates and maintenance', flags: 1 /* NotificationFlags.Ok */
            });
        });
    });
    describe('initialize()', () => {
        it('initializes all controllers', () => {
            const initialize = (0, sinon_1.stub)();
    ...
    describe('update()', () => {
        it('updates entities positions', () => {
            const e = (0, mocks_1.serverEntity)(1, 1, 1, 0);
            e.flags |= 1 /* EntityFlags.Movable */;
            e.vx = 1;
            e.vy = 1;
            e.timestamp = 1;
            (0, serverRegion_1.addEntityToRegion)(map.regions[0], e, map);
            world.update(1000, 2000);
            (0, chai_1.expect)(e.x).equal(2);
            (0, chai_1.expect)(e.y).equal(2);
        });
        it('does not update entity position if timestamp is in the future', () => {
            ...
            (0, serverRegion_1.addEntityToRegion)(map.regions[0], e, map);
            world.update(1000, 2000);
            (0, chai_1.expect)(e.x).equal(1);
            (0, chai_1.expect)(e.y).equal(1);
        });
        // it('updates regions', () => {
        ...
        it('commits region updates', () => {
            ...
            (0, serverRegion_1.pushUpdateEntityToRegion)(region, { entity, flags: 1 /* UpdateFlags.Position */ });
            region.clients.push(client);
            client.regions.push(region);
            map.regions = [region];
            const update = (0, sinon_1.stub)(client, 'update');
            world.update(123, 123000);
            (0, chai_1.expect)(update);
        });
        it('joins queued clients', () => {
            const client = (0, mocks_1.mockClient)();
            world.joinClientToQueue(client);
            const joinClientToWorld = (0, sinon_1.stub)(world, 'joinClientToWorld');
            world.update(123, 123000);
    ...
    describe('kick()', () => {
        ...
        it('does nothing for undefined', () => {
            (0, chai_1.expect)(world.kick(undefined)).false;
        });
        it('returns true if kicked', () => {
            const client = (0, mocks_1.mockClient)();
            (0, chai_1.expect)(world.kick(client)).true;
        });
        ...
        it('sets client leave reason', () => {
            const client = (0, mocks_1.mockClient)();
            world.kick(client);
            (0, chai_1.expect)(client.leaveReason).equal('kicked');
        });
        it('disconnects client after timeout', () => {
            const client = (0, mocks_1.mockClient)({ isConnected: true });
        ...
        it('skips disconnecting if client already disconnected', () => {
            const client = (0, mocks_1.mockClient)({ isConnected: false });
            const disconnect = (0, sinon_1.stub)(client, 'disconnect');
            world.kick(client);
            clock.tick(201);
    ...
    describe('kickAll()', () => {
        ...
        it('kicks all clients', () => {
            const a = (0, mocks_1.mockClient)();
            const b = (0, mocks_1.mockClient)();
            world.clients.push(a);
            world.clients.push(b);
            const kick = (0, sinon_1.stub)(world, 'kick');
            world.kickAll();
        ...
        it('works while removing clients', () => {
            ...
            world.clients.push(a);
            world.clients.push(b);
            const aLeft = (0, sinon_1.stub)(a, 'left');
            const bLeft = (0, sinon_1.stub)(b, 'left');
            world.kickAll();
