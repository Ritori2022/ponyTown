...
describe('LiveCollection', () => {
    ...
    describe('.push()', () => {
        it('adds item to collection', () => {
            ...
            (0, chai_1.expect)(collection.items).contain(item);
            (0, chai_1.expect)(collection.get('foo')).equal(item);
        });
    });
    describe('.remove()', () => {
        it('sends remove request to server', () => {
            const removeItem = (0, sinon_1.stub)(socket.server, 'removeItem').returns(Promise.resolve());
            return collection.remove('foo')
                .then(() => sinon_1.assert.calledWith(removeItem, 'events', 'foo'));
        });
        it('removes item from list', () => {
            const item = { _id: 'foo', name: 'Foo', updatedAt: new Date() };
            (0, sinon_1.stub)(socket.server, 'removeItem').returns(Promise.resolve());
            options.deleteItems = true;
            collection.push(item);
            return collection.remove('foo')
                .then(() => {
                (0, chai_1.expect)(collection.items).not.contain(item);
                (0, chai_1.expect)(collection.get('foo')).undefined;
            });
        });
    });
    describe('.removeItem()', () => {
        ...
        it('does not remove item is deleteItems flag is set to false', () => {
            ...
            (0, chai_1.expect)(collection.items).contain(item);
            (0, chai_1.expect)(item.deleted).true;
        });
        it('does nothing deleteItems flag is set to false and deleted flag is false', () => {
            ...
            collection.removeItem('foo');
            (0, chai_1.expect)(collection.items).contain(item);
            (0, chai_1.expect)(item.deleted).not.true;
        });
    });
    describe('.assignAccount()', () => {
        it('send assign account request to server', () => {
            const assignAccount = (0, sinon_1.stub)(socket.server, 'assignAccount').returns(Promise.resolve());
    ...
    describe('.live()', () => {
        ...
        it('schedules next update', () => {
            return collection.live()
                .then(() => {
                const live = (0, sinon_1.stub)(collection, 'live');
                clock.tick(2100);
                sinon_1.assert.calledOnce(live);
            });
        });
        it('schedules next update fast if more flag is true', () => {
            (0, sinon_1.stub)(socket.server, 'getAll').resolves({});
            (0, sinon_1.stub)(collection, 'read').returns(true);
        ...
        it('polls server for updates if connected', () => {
            const data = {};
            (0, sinon_1.stub)(socket.server, 'getAll').withArgs('events', '2010-01-01T10:00:00.000Z').resolves(data);
            const read = (0, sinon_1.stub)(collection, 'read');
            socket.isConnected = true;
            return collection.live()
                .then(() => sinon_1.assert.calledWith(read, data));
        });
        it('does not poll the server if not connected', () => {
            const getAll = (0, sinon_1.stub)(socket.server, 'getAll');
            (0, sinon_1.stub)(collection, 'read');
        ...
        it('does not reject for server error', async () => {
            (0, sinon_1.stub)(socket.server, 'getAll').rejects(new Error('test'));
            socket.isConnected = true;
            await collection.live();
        });
    });
    describe('.read()', () => {
        ...
        it('returns more flag', () => {
            (0, chai_1.expect)(collection.read({ ...base, more: false })).false;
            (0, chai_1.expect)(collection.read({ ...base, more: true })).true;
        });
        it('updates timestamp', async () => {
            const getAll = (0, sinon_1.stub)(socket.server, 'getAll').resolves({});
            socket.isConnected = true;
            collection.read({
                ...base,
                updates: [
        ...
        it('adds new items', () => {
            ...
            (0, chai_1.expect)(collection.items[0]).eql({
                _id: 'foo',
                name: 'Foo',
        ...
        it('does not add new items if ignored', () => {
            ...
            (0, chai_1.expect)(collection.items).empty;
        });
        ...
        it('calls beforeUpdate hook with items', () => {
            const beforeUpdate = (0, sinon_1.stub)();
        ...
        it('calls onUpdated hook with added and all items', () => {
            const onUpdated = (0, sinon_1.stub)();
            ...
            (0, chai_1.expect)(onUpdated.args[0][0]).eql([
                { _id: 'bar', name: 'Bar', updatedAt: new Date('2010-01-04T10:00:00.000Z') },
            ], 'added');
            (0, chai_1.expect)(onUpdated.args[0][1]).eql([
                { _id: 'bar', name: 'Bar', updatedAt: new Date('2010-01-04T10:00:00.000Z') },
        ...
        it('calls onFinished hook when more flag is false', () => {
            const onFinished = (0, sinon_1.stub)();
