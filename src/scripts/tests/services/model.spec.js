...
describe.skip('Model', () => {
    let http = (0, lib_1.stubClass)(http_1.HttpClient);
    let router = (0, lib_1.stubClass)(router_1.Router);
    ...
    let model;
    beforeEach(async () => {
        (0, lib_1.resetStubMethods)(http, 'post', 'get');
        (0, lib_1.resetStubMethods)(router);
        (0, lib_1.resetStubMethods)(storage);
        (0, lib_1.resetStubMethods)(errorReporter);
        http.post.withArgs('/api1/account').returns((0, rxjs_1.from)([{
                id: 'accid',
                name: 'accname',
                settings: {},
                friends: [],
    ...
    describe('supporter', () => {
        it('returns 0 if not supporter', () => {
            (0, chai_1.expect)(model.supporter).equal(0);
        });
        it('returns supporter level', () => {
            model.account.supporter = 2;
            (0, chai_1.expect)(model.supporter).equal(2);
        });
    });
    describe('signOut()', () => {
        it('makes sign out request', async () => {
            http.post.withArgs('/auth/sign-out').returns((0, rxjs_1.from)([{}]));
            await model.signOut();
            await model.accountPromise;
        ...
        it('re-fetches account details', async () => {
            http.post.reset();
            http.post.withArgs('/auth/sign-out').returns((0, rxjs_1.from)([{}]));
            http.post.withArgs('/api1/account').returns((0, rxjs_1.from)([null]));
            await model.signOut();
            await model.accountPromise;
    ...
    describe('updateAccount()', () => {
        it('makes update request', async () => {
            http.post.withArgs('/api/account-update').returns((0, rxjs_1.from)([{ name: 'bar' }]));
        ...
        it('updates account with returned result', async () => {
            http.post.withArgs('/api/account-update').returns((0, rxjs_1.from)([{ name: 'bar' }]));
            await model.updateAccount({ name: 'bar', birthdate: '' });
            (0, chai_1.expect)(model.account.name).equal('bar');
        });
    });
    describe('saveSettings()', () => {
        it('makes save settings request', async () => {
            http.post.withArgs('/api/account-settings')
                .returns((0, rxjs_1.from)([{ settings: { ignorePartyInvites: true } }]));
        ...
        it('updates settings with returned result', async () => {
            http.post.withArgs('/api/account-settings')
                .returns((0, rxjs_1.from)([{ settings: { ignorePartyInvites: true } }]));
            await model.saveSettings({ ignorePartyInvites: true });
            (0, chai_1.expect)(model.account.settings.ignorePartyInvites).true;
        });
    });
    describe('removeSite()', () => {
        it('makes remove request', async () => {
            http.post.withArgs('/api/remove-site').returns((0, rxjs_1.from)([{}]));
        ...
        it('removes site from the list', async () => {
            ...
            await model.removeSite('siteid');
            (0, chai_1.expect)(model.account.sites).not.contain(site);
        });
    });
    describe('savePony()', () => {
        it('makes save request', async () => {
            http.post.withArgs('/api/pony/save').returns((0, rxjs_1.from)([{}]));
        ...
        it('rejects if name is invalid', async () => {
            http.post.withArgs('/api/pony/save').returns((0, rxjs_1.from)([{}]));
            await (0, chai_1.expect)(model.savePony({ id: '', name: '', info: 'ponyinfo' })).rejectedWith(errors_1.NAME_ERROR);
        });
        it('rejects if already saving', async () => {
            http.post.withArgs('/api/pony/save').returns((0, rxjs_1.from)([{}]));
            await Promise.all([
                model.savePony({ id: '', name: 'ponyname', info: 'ponyinfo' }),
                (0, chai_1.expect)(model.savePony({ id: '', name: '', info: 'ponyinfo' })).rejectedWith('Saving in progress'),
            ]);
        });
    });
    describe('removePony()', () => {
        it('makes remove request', async () => {
            ...
            http.post.withArgs('/api/pony/remove').returns((0, rxjs_1.from)([{}]));
    ...
    describe('status()', () => {
        it('fetches full server status', async () => {
            const status = {};
            http.get.withArgs('/api2/game/status').returns((0, rxjs_1.from)([status]));
            const result = await model.status(false);
            sinon_1.assert.calledWithMatch(http.get, '/api2/game/status');
            (0, chai_1.expect)(result).eql(status);
        });
    });
    describe('join()', () => {
        it('fetches join result', async () => {
            const state = {};
            http.post.withArgs('/api/game/join').returns((0, rxjs_1.from)([state]));
            ...
            (0, chai_1.expect)(result).equal(state);
        });
        it('rejects if called when already pending', async () => {
            http.post.withArgs('/api/game/join').returns((0, rxjs_1.from)([{}]));
            const promise1 = model.join('serverid', 'ponyid');
            const promise2 = model.join('serverid', 'ponyid');
            await Promise.all([
                promise1,
                (0, chai_1.expect)(promise2).rejectedWith('Joining in progress'),
            ]);
        });
        it('rejects if server is invalid', async () => {
            http.post.withArgs('/api/game/join').returns((0, rxjs_1.from)([{}]));
            await (0, chai_1.expect)(model.join('', 'ponyid')).rejectedWith('Invalid server ID');
        });
        it('rejects if pony is invalid', async () => {
            http.post.withArgs('/api/game/join').returns((0, rxjs_1.from)([{}]));
            await (0, chai_1.expect)(model.join('serverid', '')).rejectedWith('Invalid pony ID');
        });
    });
});
