...
describe('LiveList', () => {
    ...
    beforeEach(() => {
        model = {
            find: (0, sinon_1.stub)(),
            findById: (0, sinon_1.stub)(),
            update: (0, sinon_1.stub)(),
    ...
    it('returns loaded state', () => {
        ...
        (0, chai_1.expect)(liveList.loaded).true;
        liveList.finished = false;
        (0, chai_1.expect)(liveList.loaded).false;
    });
    describe('for()', () => {
        it('calls callback for item', () => {
            const callback = (0, sinon_1.stub)();
            const item = { _id: 'foo' };
            liveList.add(item);
    ...
    describe('add()', () => {
        it('adds item to items', () => {
            const item = { _id: 'foo' };
            liveList.add(item);
            (0, chai_1.expect)(liveList.items).contain(item);
        });
        it('adds item to item map', () => {
            const item = { _id: 'foo' };
            liveList.add(item);
            (0, chai_1.expect)(liveList.get('foo')).equal(item);
        });
        it('triggers event listeners', () => {
            const item = { _id: 'foo' };
            const listener = (0, sinon_1.stub)();
    ...
    describe('removed()', () => {
        it('removed item from items', () => {
            ...
            (0, chai_1.expect)(liveList.items).not.contain(item);
        });
        it('removed item from items map', () => {
            const item = { _id: 'foo' };
            liveList.add(item);
            liveList.removed('foo');
            (0, chai_1.expect)(liveList.get('foo')).undefined;
        });
        it('triggers event listeners', () => {
            const item = { _id: 'foo' };
            liveList.add(item);
            const trigger = (0, sinon_1.stub)(liveList, 'trigger');
            liveList.removed('foo');
    ...
    describe('trigger() / subscribe()', () => {
        it('calls all listeners', () => {
            const item = { _id: 'foo' };
            const listener1 = (0, sinon_1.stub)();
            const listener2 = (0, sinon_1.stub)();
    ...
    describe('start()', () => {
        it('starts first tick', () => {
            const tick = (0, sinon_1.stub)(liveList, 'tick');
            liveList.start();
    ...
    describe('tick()', () => {
        ...
        it('does nothing if not running', async () => {
            const update = (0, sinon_1.stub)(liveList, 'update').resolves();
            await liveList.tick();
    ...
    describe('update()', () => {
        ...
        function setupFind(items) {
            model.find.returns({ lean: (0, sinon_1.stub)().returns(createQuery(items)) });
        }
        ...
        it('calls onAddedOrUpdated event handler', async () => {
            const onAddedOrUpdated = (0, sinon_1.stub)();
        ...
        it('updates existing item', () => {
            ...
            liveList.update();
            (0, chai_1.expect)(item.value).equal(2);
        });
        it('fixes documents before updating', () => {
            ...
            setupFind([{ _id: 'foo', value: 2 }]);
            liveList.update();
            (0, chai_1.expect)(item).eql({ _id: 'foo', value: 2, field: 'bar' });
        });
